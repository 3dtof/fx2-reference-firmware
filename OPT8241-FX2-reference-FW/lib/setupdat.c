...
extern void handle_uvcrequest(void);

static BYTE config_done_flag = 0;

void set_config_done(void)
{
	config_done_flag = 1;
}
void clear_config_done(void)
{
	config_done_flag = 0;
}
...
void handle_setupdata() {
    ...
    else if (SETUPDAT[0] & CLASS_REQ ) //is this a class specific command?? -- bmRequestType
    {
...
extern code WORD dev_dscr_uvc;
extern code WORD dev_qual_dscr_uvc;
extern code WORD highspd_dscr_uvc;
extern code WORD fullspd_dscr_uvc;
extern code WORD dev_dscr_fwld;
extern code WORD dev_qual_dscr_fwld;
extern code WORD highspd_dscr_fwld;
extern code WORD fullspd_dscr_fwld;
extern code WORD dev_strings;

WORD pDevConfig = (WORD)&fullspd_dscr_uvc;
WORD pOtherConfig = (WORD)&highspd_dscr_uvc;

void handle_hispeed(BOOL highspeed) {
 __critical {
     if (highspeed) {
		if (fw_download_done == 1) {
			pDevConfig=(WORD)&highspd_dscr_uvc;
			pOtherConfig=(WORD)&fullspd_dscr_uvc;
		} else {
			pDevConfig=(WORD)&highspd_dscr_fwld;
			pOtherConfig=(WORD)&fullspd_dscr_fwld;
		}
	} else {
		if (fw_download_done == 1) {
			pDevConfig=(WORD)&fullspd_dscr_uvc;
			pOtherConfig=(WORD)&highspd_dscr_uvc;
		} else {
			pDevConfig=(WORD)&fullspd_dscr_fwld;
			pOtherConfig=(WORD)&highspd_dscr_fwld;
		}
    }
 }
}
...
void handle_get_descriptor() {
    ...
    switch ( SETUPDAT[3] ) {
        case DSCR_DEVICE_TYPE:
			...
			if (fw_download_done == 1) {
				SUDPTRH = MSB((WORD)&dev_dscr_uvc);
				SUDPTRL = LSB((WORD)&dev_dscr_uvc);
			} else {
				SUDPTRH = MSB((WORD)&dev_dscr_fwld);
				SUDPTRL = LSB((WORD)&dev_dscr_fwld);
			}
            break;
        ...
        case DSCR_DEVQUAL_TYPE:
			...
			if (fw_download_done == 1) {
				SUDPTRH = MSB((WORD)&dev_qual_dscr_uvc);
				SUDPTRL = LSB((WORD)&dev_qual_dscr_uvc);
			} else {
				SUDPTRH = MSB((WORD)&dev_qual_dscr_fwld);
				SUDPTRL = LSB((WORD)&dev_qual_dscr_fwld);
			}
            break;
